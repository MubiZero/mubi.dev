...
for (const route of ROUTES) {
  test.describe(`${route.locale} page`, () => {
    ...
    test('the portrait loads and is named', async ({ page }) => {
      const portrait = page.locator('.hero__portrait img');
    ...
    test('the portrait never crowds out the fold on a narrow screen', async ({ page }) => {
      for (const width of [360, 820]) {
        ...
        const box = await page.locator('.hero__portrait img').boundingBox();
    ...
    test('printing produces a one page resume, not a screenshot of the site', async ({ page }) => {
      ...
      await expect(page.locator('.hero .social')).toBeHidden();
      await expect(page.locator('#work')).toBeHidden();
      await expect(page.locator('.track__screen')).toBeHidden();
      await expect(page.locator('.track__print')).toBeVisible();
      await expect(page.getByTestId('contact-mail')).toBeVisible();
    ...
    test('the code section shows real repositories with live metadata', async ({ page }) => {
      ...
      for (const repo of await repos.all()) {
        ...
        await expect(repo.locator('.repo__description')).not.toBeEmpty();
        // a month and a year, filled in at build time from the GitHub API
        await expect(repo.locator('.repo__meta')).toContainText(/\d{4}/);
      }
    });
    ...
    test('the contribution calendar draws its window and names its own total', async ({
      ...
      const days = calendar.locator('.calendar__day[data-level]');
      ...
      const scroller = calendar.locator('.calendar__scroll');
    ...
    test('a day answers when you point at it', async ({ page }) => {
      const calendar = page.getByTestId('calendar');
      const tip = calendar.locator('.calendar__tip');
      await expect(tip).toBeHidden();

      await calendar.locator('.calendar__day[data-day]').last().hover();
    ...
    test('the days are walkable from the keyboard, one tab stop away', async ({ page }) => {
      const calendar = page.getByTestId('calendar');
      await calendar.locator('.calendar__scroll').focus();

      await page.keyboard.press('ArrowLeft');
      const cursor = calendar.locator('.calendar__day[data-cursor]');
      ...
      await expect(calendar.locator('[data-calendar-live]')).toHaveText(
        (await cursor.getAttribute('data-day'))!,
      );

      await page.keyboard.press('Escape');
      await expect(calendar.locator('.calendar__day[data-cursor]')).toHaveCount(0);
    });
    ...
    test('nothing rules off one section from the next', async ({ page }) => {
      const borders = await page.$$eval('.section', (nodes) =>
    ...
    test('the calendar scrolls inside itself, never widening the page', async ({ page }) => {
      ...
      const scroller = page.locator('.calendar__scroll');
